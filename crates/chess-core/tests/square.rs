...
fn creates_valid_squares() {
    let a1: Square = Square::new(0, 0).expect("a1 is valid");
    let h8: Square = Square::new(7, 7).expect("h8 is valid");
...
fn index_round_trip_preserves_every_square() {
    for index in 0..Square::COUNT {
        let square: Square = Square::from_index(index).expect("index is on the board");
